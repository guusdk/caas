...
@ComplianceTest(
        ...
        description =
                "Provides users a virtually zero overhead way to bypass restrictive firewalls "
...
public class XmppOverTls extends AbstractTest {
    ...
    public boolean run() {
        ...
        try {
            ...
            ResolverResult<SRV> results =
                    ResolverApi.INSTANCE.resolve("_xmpps-client._tcp." + domain, SRV.class);
            for (SRV record : results.getAnswers()) {
                try {
                    SSLSocket socket =
                            (SSLSocket)
                                    sslSocketFactory.createSocket(
                                            record.name.toString(), record.port);
                    socket.setSSLParameters(parameters);
                    socket.setSoTimeout(1000);
                    socket.startHandshake();
                    LOGGER.debug(
                            "attempting to verify certificate on "
                    ...
                    while (remainingChars > 0) {
                        int charsRead =
                                bufferedReader.read(
                                        buffer, buffer.length - remainingChars, remainingChars);
                        if (charsRead == -1) {
                            return false;
                        }
