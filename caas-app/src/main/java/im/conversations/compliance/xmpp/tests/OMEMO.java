 ...
 * such that everyone can access it. Without publish-options OMEMO is only available to contacts
 * with mutual presence subscription.
 */
@ComplianceTest(
        short_name = "xep0384",
...
public class OMEMO extends AbstractTest {
    ...
    private static final String PUBLISH_OPTIONS =
    ...
    private static boolean hasPepIdentity(InfoNode infoNode) {
        for (Identity identity : infoNode.getIdentities()) {
            if ("pep".equalsIgnoreCase(identity.getType())
                    && "pubsub".equalsIgnoreCase(identity.getCategory())) {
                return true;
            }
        }
        return false;
    ...
    public boolean run() {
        ...
        AsyncResult<InfoNode> result =
                manager.discoverInformation(client.getConnectedResource().asBareJid());
        try {
            ...
            if (features.contains(PUBLISH_OPTIONS)) {
                PubSubService personalEventingService =
                        client.getManager(PubSubManager.class).createPersonalEventingService();
