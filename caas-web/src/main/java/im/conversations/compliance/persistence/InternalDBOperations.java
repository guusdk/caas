...
public class InternalDBOperations {
    ...
    public static boolean addServer(Connection connection, Server server) {
        String query =
                "insert into servers(domain,listed,software_name,software_version)"
                        + " values(:domain,:listed,:softwareName,:softwareVersion)";
    ...
    public static boolean setListed(Connection connection, String domain, boolean listedServer) {
        String query = "update servers set" + " listed=:listed" + " where domain=:domain";
        connection
                .createQuery(query)
    ...
    public static boolean updateServer(Connection connection, Server newServer) {
        String query =
                "update servers set"
                        ...
                        + " where domain=:domain";
        connection.createQuery(query).bind(newServer).executeUpdate();
        return true;
    }

    public static List<String> getCompliantServers(Connection connection) {
        String query =
                "select servers.domain from servers join credentials on"
                    + " servers.domain=credentials.domain inner join current_tests on"
                    + " current_tests.domain = servers.domain where listed=1 and test in (:tests)"
                    + " group by current_tests.domain having sum(success) = count(success)";
    ...
    public static List<Server> getPublicServers(Connection connection) {
        String query =
                "select servers.domain,software_name,software_version,listed from servers join"
                    + " credentials on servers.domain=credentials.domain where listed=1 order by"
                    + " servers.domain";
        return connection
                .createQuery(query)
    ...
    public static List<Server> getAllServers(Connection connection) {
        String query =
                "select servers.domain,software_name,software_version,listed from servers join"
                    + " credentials on servers.domain=credentials.domain order by servers.domain";
        return connection
                .createQuery(query)
                .addColumnMapping("software_name", "softwareName")
                .addColumnMapping("software_version", "softwareVersion")
    ...
    public static Server getServer(Connection connection, String domain) {
        String query =
                "select domain,software_name,software_version,listed from servers where domain ="
                        + " :domain";
        return connection
                .createQuery(query)
                .addColumnMapping("software_name", "softwareName")
    ...
    public static Iteration getLatestIteration(Connection connection) {
        String query =
                "select iteration_number, begin_time, end_time from periodic_test_iterations where"
                        + " iteration_number = (select max(iteration_number) from"
                        + " periodic_test_iterations)";
        return connection
                .createQuery(query)
    ...
    public static Map<String, List<HistoricalSnapshot>> getHistoricalSnapshotsGroupedByServer(
            Connection connection, List<String> tests, List<String> domains) {
        ...
        for (String domain : domains) {
            ...
            for (String test : tests) {
                Table table =
                        connection
                                .createQuery(
                                        "select iteration_number,success from periodic_tests "
            ...
            String query =
                    "select count(success),sum(success) from periodic_tests where iteration_number"
                            + " = :it and domain=:domain and test in (:tests)";
            ...
            for (int changeIterations : resultChanges.keySet()) {
                ...
                String timestamp = getIteration(connection, changeIterations).getBegin().toString();
                historicalSnapshots.add(
                        new HistoricalSnapshot(
                                ...
                                pass,
                                total,
                                resultChanges.get(changeIterations)));
            }
    ...
    /**
     * Get points at which there is a change in the periodic results, along with a corresponding
     * list of all the changes. It takes all periodic results for a particular test-domain pair and
     * adds all the points at which there is a change along with the value taken as a parameter
     *
     * @param changes The map to which all the changes will be added
     * @param value The value which has to be stored corresponding to this change (when grouping by
     *     test, it is server's domain name and vice versa)
     * @param iterationResultPairs A list of rows of results for a particular domain and test
     */
    private static void addHistoricalPoints(
            HashMap<Integer, HistoricalSnapshot.Change> changes,
            String value,
            List<Row> iterationResultPairs) {
        int lastResult = -1;
     ...
     *
     * @param connection
     * @param iterationNumber The iteration numbers for which the corresponding iteration is to be
     *     found
     * @return A list of iterations
     */
    public static Iteration getIteration(Connection connection, int iterationNumber) {
        String query =
                "select iteration_number, begin_time, end_time from periodic_test_iterations"
    ...
    public static HashMap<String, List<HistoricalSnapshot>> getHistoricalSnapshotGroupedByTest(
            Connection connection, List<String> tests, List<String> domains) {
        ...
        for (String test : tests) {
            ...
            String query =
                    "select count(success),sum(success) from periodic_tests where iteration_number"
                            + " = :it and test=:test and domain in (:domains)";
    ...
    /**
     * @param connection should be a transaction of type
     *     java.sql.Connection.TRANSACTION_SERIALIZABLE for sqlite database
     * @param rdpList
     * @param beginTime
     * @param endTime
     * @return
     */
    public static boolean addPeriodicResults(
            Connection connection,
            List<ResultDomainPair> rdpList,
            Instant beginTime,
            Instant endTime) {
        // Add the iteration to iterations list
        String query =
                "insert into periodic_tests(domain,test,success,iteration_number)"
                    ...
                    rdp.getResults()
                            .forEach(
                                    result ->
                                            resultInsertQuery
                                                    .addParameter(
                                                            "test", result.getTest().short_name())
    ...
    private static void addIteration(
            Connection connection, int iterationNumber, Instant beginTime, Instant endTime) {
        String query =
                "insert into periodic_test_iterations(iteration_number,begin_time,end_time) "
     ...
     * @return
     */
    public static boolean addCurrentResults(
            Connection connection, String domain, List<Result> results, Instant timestamp) {
        String queryText =
                "replace into current_tests(domain,test,success,timestamp) "
                        + "values(:domain,:test,:success,:timestamp)";
        Query query = connection.createQuery(queryText);
        results.forEach(
                result ->
                        query.addParameter("test", result.getTest().short_name())
                                .addParameter("success", result.isSuccess())
                                .addParameter("domain", domain)
    ...
    public static boolean addCredential(Connection connection, Credential credential) {
        final String query =
                "insert into credentials(domain,jid,password,failures)"
                        + " values(:domain,:jid,:password,0)";
    ...
    public static Map<String, HashMap<String, Boolean>> getCurrentResultsByServer(
        ...
        Table table =
                connection
                        .createQuery(
                                "select test,servers.domain,success from current_tests inner join"
                                        + " servers on servers.domain = current_tests.domain join"
                                        + " credentials on servers.domain=credentials.domain  where"
                                        + " test in (:tests) and listed = 1 order by (select"
                                        + " sum(success) from current_tests where"
                                        + " domain=servers.domain and test in (:tests))desc,"
                                        + " servers.domain asc")
                        .addParameter("tests", tests)
                        .executeAndFetchTable();
        table.rows()
    ...
    public static List<Result> getCurrentResultsForServer(Connection connection, String domain) {
        ...
        String query =
                "select test,success from current_tests" + " where domain = :domain order by test";
        Table table =
                connection.createQuery(query).addParameter("domain", domain).executeAndFetchTable();
        table.rows()
                .forEach(
                        row -> {
                            String test = row.getString("test");
    ...
    public static Map<String, Boolean> getCurrentResultsForTest(
            Connection connection, String test) {
        String query =
                "select servers.domain,success from current_tests"
                        + " inner join servers on servers.domain = current_tests.domain"
                        + " where test = :test and listed = 1";
        Table table =
                connection.createQuery(query).addParameter("test", test).executeAndFetchTable();
    ...
    public static Map<String, HashMap<String, Boolean>> getCurrentResultsByTest(
        ...
        Table table =
                connection
                        .createQuery(
                                "select test,servers.domain,success from current_tests inner join"
                                        + " servers on servers.domain = current_tests.domain where"
                                        + " test in (:tests) and listed=1")
                        .addParameter("tests", tests)
                        .executeAndFetchTable();
        table.rows()
                .forEach(
    ...
    public static Instant getLastRunFor(Connection connection, String domain) {
        Instant lastRun =
                connection
                        .createQuery(
                                "select max(timestamp) from current_tests where domain=:domain")
                        .addParameter("domain", domain)
                        .executeScalar(Instant.class);
        return lastRun;
    }

    public static Map<String, HashMap<String, Boolean>> getHistoricalTableFor(
            Connection connection, int iterationNumber) {
        Map<String, HashMap<String, Boolean>> resultsByServer = new LinkedHashMap<>();
        List<String> tests = TestUtils.getTestNames();
        Table table =
                connection
                        .createQuery(
                                "select test,servers.domain,success from periodic_tests inner join"
                                        + " servers on servers.domain = periodic_tests.domain where"
                                        + " test in (:tests) and listed = 1 and iteration_number ="
                                        + " :iteration_number order by (select sum(success) from"
                                        + " periodic_tests where domain=servers.domain and test in"
                                        + " (:tests) and iteration_number=:iteration_number ) desc,"
                                        + " servers.domain asc")
    ...
    public static List<Result> getHistoricalResultsFor(
            Connection connection, String domain, int iteration) {
        Table table =
                connection
                        .createQuery(
                                "select test,success from periodic_tests where domain=:domain and"
                                        + " iteration_number = :iteration order by test")
                        .addParameter("domain", domain)
    ...
    public static boolean addSubscriber(Connection connection, Subscriber subscriber) {
        connection
                .createQuery(
                        "insert into subscribers(domain,email,unsubscribeCode) "
    ...
    public static List<Subscriber> getSubscribersFor(Connection connection, String domain) {
        List<Subscriber> subscribers =
                connection
                        .createQuery(
                                "select email,domain,unsubscribeCode from subscribers where"
                                        + " domain=:domain")
                        .addParameter("domain", domain)
    ...
    public static Subscriber removeSubscriber(Connection connection, String unsubscribeCode) {
        ...
        connection
                .createQuery("delete from subscribers " + "where unsubscribeCode=:unsubscribeCode")
                .addParameter("unsubscribeCode", unsubscribeCode)
                .executeUpdate();
    ...
    public static int deleteFailedCredentials(Connection connection) {
        return connection
                .createQuery("DELETE from credentials WHERE failures > 2")
                .executeUpdate()
                .getResult();
    }
}
